...
if(cmd === `!ip`){
	...
	.setTitle("☆Loot++ ☆Clans ☆Friends ☆AutoDoors ☆BGrade ☆CCTV Cameras ☆Cars ☆InstaCraft ☆QuickSmelt ☆Teleport ☆Homes ☆No Blueprints ☆No Workbench")
	.setColor(0x30bdff)
	.addField("195.201.80.230:28716", "client.connect 195.201.80.230:28716", false)
	message.channel.send({embed});
}
if(cmd === `!vote`){
...
if(cmd === `!wipe`){
	return message.channel.send("The last wipe was on **Thu 8 Aug **. The next wipe will be on **Thu 22 Aug**. (Around 19:00 CET.)");
}
if(cmd === `!steam`){
...
if(message.channel.id === "557278884052205568"){
	...
	if (command === "saygiveaway") {
		const color = args[0]
		const text = args.slice(0).join(" ");
		let channel = client.channels.find("id", "503609273142738967");
		if (text.length < 1) return message.channel.send("Can not announce nothing");
			channel.send(text)
	} else
	if (command === "sayall") {
